...
function App() {
  ...
  const acceptAI = () => { if (!aiModel) return; setForm(aiModel.problem); setMode('advanced'); setAiModel(null) }
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {aiModel && <section className="card ai-review">
        ...
        <div className="grid two"><div className="info"><span>Problem family</span><strong>{aiModel.problem.problem_family}</strong></div><div className="info"><span>Objective</span><strong>{aiModel.problem.objective_sense} · {aiModel.problem.expression}</strong></div></div>
        ...
        <div className="actions"><button className="ghost" onClick={()=>{setForm(aiModel.problem);setAiModel(null);setMode('advanced')}}><Settings2 size={16}/> Edit model</button><button className="primary compact" onClick={()=>{setForm(aiModel.problem);setAiModel(null);setMode('advanced');setTimeout(validate,0)}}>Accept & validate <ArrowRight size={17}/></button></div>
      </section>}
