  ...
  return <div className="shell">
    ...
    <main>
      ...
      {aiModel && <section className="card ai-review">
        <div className="section-head"><span className="step">02</span><div><h2>How Optimization Lab understood your problem</h2><p>Review the proposed model in plain language before validating it. You can edit anything that does not match what you intended.</p></div></div>
        <div className="ai-summary"><Sparkles size={18}/><div><strong>What the model does</strong><p>{aiModel.explanation}</p></div></div>
        <div className="grid two"><div className="info"><span>Problem type</span><strong>{aiModel.problem.problem_family}</strong></div><div className="info"><span>What is being optimized</span><strong>{aiModel.problem.objective_sense}{aiModel.problem.expression ? ` · ${aiModel.problem.expression}` : aiModel.problem.objective_metric ? ` · ${aiModel.problem.objective_metric}` : ' · Not yet determined'}</strong></div></div>
        <div className="grid two"><div className="info"><span>What the model decides</span><strong>{aiModel.problem.variables.length > 0 ? aiModel.problem.variables.map(v=>v.name).join(', ') : 'Not determined'}</strong></div><div className="info"><span>Model status</span><strong>{aiModel.incomplete ? 'Incomplete — needs review' : 'Complete proposal'}</strong></div></div>
        <div className="info"><span>Model characteristics</span><div className="chips readonly">{aiModel.problem.mathematical_properties.map(p=><span className="chip active" key={p}>{p}</span>)}</div></div>
        <div className="dataset-card"><div><FileText size={17}/><strong>Data used · {aiModel.datasets.length} source{aiModel.datasets.length===1?'':'s'}</strong><span>{aiModel.datasets.map(source => source.source_name || source.filename).join(' · ')}</span></div></div>
        ...
        {aiModel.assumptions.length>0 && <div className="assumptions"><AlertTriangle size={16}/><div><strong>Things to review</strong><p className="assumptions-intro">These are modeling choices or details that were not completely defined by the description or data.</p>{aiModel.assumptions.map(a=><div key={a}>• {a}</div>)}</div></div>}
